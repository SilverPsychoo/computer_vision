...
# Convertir target
data["Personality"] = (data["Personality"]=="Extrovert").astype(int)

# Forzar columnas a numéricas
data = data.apply(pd.to_numeric, errors="coerce")
data = data.dropna()

X = data.drop("Personality",axis=1).values.astype(np.float32)
y = data["Personality"].values.astype(np.int32)
